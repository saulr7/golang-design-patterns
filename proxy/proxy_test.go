...
func Test_UserListProxy(t *testing.T) {
	...
	t.Run("Finde User - Empty cache", func(t *testing.T) {
		...
		if proxy.DidDidLastSearchUsedCache {
			t.Error("No user can be returned from an empty cache")
		}
	})

	t.Run("FindeUser - One user, ask for the same user", func(t *testing.T) {
